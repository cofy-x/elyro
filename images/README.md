...
Append `-arm64` to a target for a local arm64 build. Source builds use architecture-specific tags such as `elyro/workspace-go:dev-arm64`; public releases publish exact multi-architecture tags such as `ghcr.io/cofy-x/elyro/workspace-go:v0.9.0`.
...
```bash
make image-report VERSION=v0.9.0
make image-report VERSION=v0.9.0 COMPARE_VERSION=v0.8.0 FORMAT=json
make image-report VERSION=v0.9.0 TOP_LAYERS=10
```
