...
Append `-arm64` to a target for a local arm64 build. Source builds use architecture-specific tags such as `elyro/workspace-go:dev-arm64`; public releases publish exact multi-architecture tags such as `ghcr.io/cofy-x/elyro/workspace-go:v0.1.6`.
...
```bash
make image-report VERSION=v0.1.6
make image-report VERSION=v0.1.6 COMPARE_VERSION=v0.1.5 FORMAT=json
make image-report VERSION=v0.1.6 TOP_LAYERS=10
```
